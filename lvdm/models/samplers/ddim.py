...
class DDIMSampler(object):
    ...
    #     samples, intermediates = self.ddim_search_sampling(conditioning, size,
    #                                                 model,
    #                                                 search_k,
    #                                                 search_steps,
    #                                                 callback=callback,
    ...
    @torch.no_grad()
    def ddim_search_sampling(self, model, search_k, search_steps, cond, shape,
        ...
        if x_T is None:
            ...
            img = x_T
        img = torch.randn((search_k, c, f, h, w), device=device)
        ...
        # print("timesrange:",time_range)
        # print("timesrange.shape:",time_range.shape)
        ...
        for i, step in enumerate(iterator):
            ...
            if i == 0:
                for j in range(search_k):
                    outs = self.p_sample_ddim(img[j].unsqueeze(0), cond, ts, index=index, use_original_steps=ddim_use_original_steps,
                ...
                img = selected_img(x0_list, img_list)
                continue
    ...
    def save_gif(self,batch_tensors, savedir, name):
    ...
    @torch.no_grad()
    def fifo_search_onestep(self, cond, shape, latents=None, timesteps=None, indices=None,
                     unconditional_guidance_scale=1., unconditional_conditioning=None,
                     **kwargs):
        device = self.model.betas.device        
        b, _, f, _, _ = shape
    ...
    @torch.no_grad()
    def fifo_chunk_onestep(self, cond, shape, latents=None, timesteps=None, indices=None,
                     unconditional_guidance_scale=1., unconditional_conditioning=None,
                     **kwargs):
        device = self.model.betas.device        
        b, _, f, _, _ = shape
        ...
        noise_pred = self.unet(latents, cond, ts,
                                ...
                                **kwargs)
        # if fusion and:
        #     noise_pred = noise_pred * weight + pre_noise_pred * (1 - weight)
        #     latents, pred_x0 = self.ddim_step(latents, noise_pred, indices)
        # else:
        #     latents, pred_x0 = self.ddim_step(latents, noise_pred, indices)
        return noise_pred
