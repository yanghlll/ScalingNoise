...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, context=None, features_adapter=None, fps=16, **kwargs):
        ...
        # t_emb = timestep_embedding(timesteps.flatten(), self.model_channels, repeat_only=False).reshape(b * t,-1)
        ...
        if self.fps_cond:
            ...
            # fps_emb = timestep_embedding(fps.unsqueeze(0).repeat(1, t,1).flatten(),self.model_channels, repeat_only=False).reshape(b * t, -1)
        ...
        # logger.info("input_blocks")
        for id, module in enumerate(self.input_blocks):
            # print(f"unet的里面的第几个module{id}")
            #每次取一个module
            h = module(h, emb, context=context, batch_size=b)
            #这里的addition_attention是在干嘛
            if id ==0 and self.addition_attention:
                h = self.init_attn(h, emb, context=context, batch_size=b)
            ## plug-in adapter features
            #这里的adapter features是在干嘛
        ...
        # logger.info("middle_blocks")
        h = self.middle_block(h, emb, context=context, batch_size=b)

        # logger.info("output_blocks")
