...
import copy
import random
from .reward_generated import subject_consistency
from .resample_action import random_resample, last_frame_renoise, fft_2D_resample, last_frame_resample
from tqdm import tqdm


def prepare_latents(args, device, latents_dir, sampler):
    latents_list = []
    # device = torch.cuda.current_device()
    video = torch.load(latents_dir+f"/{args.num_inference_steps}.pt", map_location='cuda:0')
    ...
    if args.lookahead_denoising:
        for i in range(args.video_length // 2):
            ...
            beta = 1 - alpha
            latents = alpha**(0.5) * video[:,:,[0]] + beta**(0.5) * torch.randn_like(video[:,:,[0]], device=device)
    ...
    for i in range(args.num_inference_steps):
        ...
        latents = (alpha)**(0.5) * video[:,:,[frame_idx]] + (1-alpha)**(0.5) * torch.randn_like(video[:,:,[frame_idx]], device=device)
        latents_list.append(latents)
...
def compute_score_video(args, prompt, x0, X0_video, device):
    subject_consistency_score = subject_consistency(X0_video, device)
    score = subject_consistency_score

    return score


def select_x0_id(args, prompt, x0_latent_list, x0_video_list, candidates, beam_latents, prev_score, e=0.2):
    score_list = [[] for _ in range(len(x0_latent_list))]
    for i in range(len(x0_latent_list)):
        for j in range(len(x0_latent_list[i])):
            score_list[i].append(compute_score_video(args, prompt, x0_latent_list[i][j], x0_video_list[i][j], beam_latents[i].device))
    flat_scores = []
    for i, scores in enumerate(score_list):
        for j, score in enumerate(scores):
            flat_scores.append((score, i, j))
    flat_scores.sort(reverse=True)
    if random.random() < 1 - e:  
        top_indices = [(i, j) for _, i, j in flat_scores[:args.m]]
    else:
        top_indices = []
        for i in range(len(x0_latent_list)):
            best_candidate = max(enumerate(score_list[i]), key=lambda x: x[1])
            top_indices.append((i, best_candidate[0]))
    idx = [i for _, i, _ in flat_scores[:args.m]]
    beam_latents = [beam_latents[i].clone() for i in idx]

    for i in range(args.m):
        beam_latents[i][:,:,-8:] = candidates[top_indices[i][0]][top_indices[i][1]][:,:,-8:].clone()
    
    return beam_latents, idx


def fifo_ddim_beamsearch(args, prompt, model, conditioning, noise_shape, ddim_sampler,\
                        cfg_scale=1.0, output_dir=None, latents_dir=None, save_frames=False, **kwargs):
    batch_size = noise_shape[0]
    b, c, f, h, w = noise_shape
    kwargs.update({"clean_cond": True})
    k = args.k
    m = args.m
    # check condition bs
    ...
    device = model.device
    latents = prepare_latents(args, device,latents_dir, ddim_sampler)
    ...
    fifo_video_frames = [[] for _ in range(m)]
    beam_latents = [latents.clone() for _ in range(m)]
    prev_score = [0 for _ in range(m)]

    action_list = [random_resample, last_frame_renoise, fft_2D_resample, last_frame_resample]
    ...
    for i in trange(args.new_video_length + args.num_inference_steps - args.video_length, desc="fifo sampling"):
        x0_latent_list = [[] for _ in range(m)]
        x0_video_list = [[] for _ in range(m)]
        ...
        if i == 0:
            for rank in reversed(range(2 * args.num_partitions if args.lookahead_denoising else args.num_partitions)):
                ...
                if args.lookahead_denoising:
                    for j in range(m):
                        beam_latents[j][:,:,midpoint_idx:end_idx] = output_latents[:,:,-(num_frames_per_gpu//2):]
                else:
                    for j in range(m):
                        beam_latents[j][:,:,start_idx:end_idx] = output_latents
                del output_latents
            first_frame_idx = args.video_length // 2 if args.lookahead_denoising else 0
            frame_tensor = model.decode_first_stage_2DAE(beam_latents[j][:,:,[first_frame_idx]]) # b,c,1,H,W

            first_image = tensor2image(frame_tensor)
            anchor_image = tensortrans(frame_tensor[0])
            if save_frames:
                fifo_path = os.path.join(fifo_dir, f"{i}.png")
                first_image.save(fifo_path)
            for j in range(m):
                fifo_video_frames[j].append(first_image)
            
            for s in range(m):
                beam_latents[s] = shift_latents(beam_latents[s])
        ...
        for j in range(m):
            for rank in reversed(range(2 * args.num_partitions if args.lookahead_denoising else args.num_partitions)):
                ...
                input_latents = beam_latents[j][:,:,start_idx:end_idx].clone()
                if rank == 2 * args.num_partitions - 1:
                    selected_action = random.choices(action_list, k=k)
                    results = []
                    
                    for action in selected_action:
                        result = action(args, b, c, h, w, input_latents[:,:,[-2]], ddim_sampler).to(input_latents.device)
                        results.append(result)
                    latent_rand = torch.cat(results, dim=2).to(input_latents.device)
                    for l in range(k):
                        video_list = [anchor_image]
                        input_latents[:,:,-1] = latent_rand[:,:,l]
                        output_latents, x0 = ddim_sampler.fifo_onestep(
                                cond=cond,
                                shape=noise_shape,
                                latents=input_latents,
                                timesteps=t,
                        ...
                        x0_video = model.decode_first_stage_2DAE(x0)

                        for y in range(x0_video.shape[2]):
                            video_list.append(tensortrans(x0_video[:,:,y]))
                        image = video_list[-1]
                        x0_latent_list[j].append(image)
                        x0_video_list[j].append(video_list)
                        candidates[j].append(output_latents)
                        del x0
                else:
                    output_latents, _ = ddim_sampler.fifo_onestep(
                                                cond=cond,
        ...
        beam_latents, idx = select_x0_id(args, prompt, x0_latent_list, x0_video_list, candidates, beam_latents, prev_score)
        
        fifo_video_frames = [copy.deepcopy(fifo_video_frames[p]) for p in idx]
...
def fifo_ddim_sampling(args, model, conditioning, noise_shape, ddim_sampler,\
    ...
    for i in trange(args.new_video_length + args.num_inference_steps - args.video_length, desc="fifo sampling"):
        ...
        latents = shift_latents_action_test(args, b, c, h, w, latents, ddim_sampler)
...
def save_mp4(batch_tensors, savedir, name):
    vid_tensor = torch.squeeze(batch_tensors) # c,f,h,w
    ...
    frames = video.chunk(video.shape[0], dim=0)
    frames = [frame.squeeze(0) for frame in frames]
    savepath = os.path.join(savedir, f"{name}.mp4")
...
def tensortrans(batch_tensors):
    batch_tensors = torch.squeeze(batch_tensors)
    image = torch.clamp(batch_tensors, -1., 1.)
    image = (image + 1.0) / 2.0
    return image